...
Почему не сдалать все ноды управляющими?
При запуске дополнительных реплик сервиса, они запустятся на назных нодах? или это надо явно указывать?
...
- Затем запускаются все контейнеры.
